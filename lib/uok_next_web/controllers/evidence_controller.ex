defmodule UokNextWeb.EvidenceController do
  ...
  def create(%{assigns: %{command_context: context}} = conn, %{"id" => party_id} = params) do
    with {:ok, key} <- RequestCommand.idempotency_key(conn),
      ...
      conn
      |> ApiResponse.command(
        submit_party(candidate, party_id, version, params, context, key),
        :ok
      )
    else
  ...
  defp find_or_store_candidate(party_id, evidence_id, version, params, context, key) do
    case Evidence.get_verified_candidate(evidence_id, party_id, context) do
      {:ok, candidate} ->
        {:ok, candidate}

      {:error, %CommandError{code: "not_found"}} ->
        store_new_candidate(party_id, evidence_id, version, params, context, key)
  ...
  defp store_new_candidate(party_id, evidence_id, version, params, context, key) do
    with :ok <- Parties.preflight_evidence(party_id, version, context),
         {:ok, upload, content} <- bounded_upload(params["file"]),
         {:ok, candidate, _disposition} <-
           Evidence.store_party_candidate(
             evidence_id,
             party_id,
  ...
  defp submit_party(candidate, party_id, version, params, context, key) do
    Parties.submit_evidence(
      party_id,
      %{"evidence_id" => candidate["id"], "reason" => params["reason"]},
      version,
      context,
      IdempotencyKey.derive(key, "party-evidence-submit")
    )
  end

  # The path is generated by Plug's multipart parser and is reachable here only
  # through a real Plug.Upload struct; clients cannot deserialize this struct or
  # choose its path. Filename and content type remain untrusted metadata.
