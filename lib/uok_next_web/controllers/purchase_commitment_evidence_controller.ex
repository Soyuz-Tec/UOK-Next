defmodule UokNextWeb.PurchaseCommitmentEvidenceController do
  ...
  def create(%{assigns: %{command_context: context}} = conn, %{"id" => proposal_id} = params) do
    with {:ok, key} <- RequestCommand.idempotency_key(conn),
         ...
         {:ok, candidate} <-
           find_or_store(proposal_id, evidence_id, version, params, context, key) do
      result =
        Contracts.submit_purchase_commitment_evidence(
          ...
          version,
          context,
          IdempotencyKey.derive(key, "purchase-commitment-evidence-submit")
        )
  ...
  defp find_or_store(proposal_id, evidence_id, version, params, context, key) do
    case Evidence.get_verified_candidate(evidence_id, @subject_type, proposal_id, context) do
      {:ok, candidate} ->
        {:ok, candidate}

      {:error, %CommandError{code: "not_found"}} ->
        store_new(proposal_id, evidence_id, version, params, context, key)
  ...
  defp store_new(proposal_id, evidence_id, version, params, context, key) do
    with :ok <- Contracts.preflight_purchase_commitment_evidence(proposal_id, version, context),
  ...
  # Plug creates this temporary path; serialized clients cannot choose it.
