defmodule UokNextWeb.SupplierQuoteEvidenceController do
  ...
  def create(%{assigns: %{command_context: context}} = conn, %{"id" => quote_id} = params) do
    with {:ok, key} <- RequestCommand.idempotency_key(conn),
         ...
         {:ok, candidate} <- find_or_store(quote_id, evidence_id, version, params, context, key) do
      result =
        Sourcing.submit_quote_evidence(
          ...
          version,
          context,
          IdempotencyKey.derive(key, "supplier-quote-evidence-submit")
        )
  ...
  defp find_or_store(quote_id, evidence_id, version, params, context, key) do
    case Evidence.get_verified_candidate(evidence_id, "supplier_quote", quote_id, context) do
      {:ok, candidate} ->
        {:ok, candidate}

      {:error, %CommandError{code: "not_found"}} ->
        store_new(quote_id, evidence_id, version, params, context, key)
  ...
  defp store_new(quote_id, evidence_id, version, params, context, key) do
    with :ok <- Sourcing.preflight_quote_evidence(quote_id, version, context),
