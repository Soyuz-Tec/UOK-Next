defmodule UokNextWeb.ReadinessEvidenceController do
  ...
  def create(%{assigns: %{command_context: context}} = conn, %{"id" => readiness_id} = params) do
    with {:ok, key} <- RequestCommand.idempotency_key(conn),
         ...
         {:ok, candidate} <-
           find_or_store(readiness_id, evidence_id, version, params, context, key) do
      result =
        Shipments.submit_readiness_evidence(
          ...
          version,
          context,
          IdempotencyKey.derive(key, "shipment-readiness-evidence-submit")
        )
  ...
  defp find_or_store(readiness_id, evidence_id, version, params, context, key) do
    case Evidence.get_verified_candidate(evidence_id, @subject_type, readiness_id, context) do
      {:ok, candidate} ->
        {:ok, candidate}

      {:error, %CommandError{code: "not_found"}} ->
        store_new(readiness_id, evidence_id, version, params, context, key)
  ...
  defp store_new(readiness_id, evidence_id, version, params, context, key) do
    with :ok <- Shipments.preflight_readiness_evidence(readiness_id, version, context),
